...
class PolicyIteration(AbstractAgent):
    ...
    def __init__(
        ...
        **kwargs: dict,
        ...
        self.pi = np.random.choice(self.A, size=len(self.S))  # randomly select an action for each state
    ...
    def update_agent(self, *args: tuple, **kwargs: dict) -> None:
        ...
        if not self.policy_fitted:
            ...
            Q_new, pi_new, steps = policy_iteration(self.Q, self.pi, (self.S, self.A, self.T, self.R_sa, self.gamma))
            self.Q = Q_new
            self.pi = pi_new
...
def policy_evaluation(
    ...
    while improvement > epsilon:
        ...
        for s in range(nS):
            v_old = V[s]
            a = pi[s]
            V[s] = R_sa[s, a] + gamma * sum([T[s, a, s_next] * V[s_next] for s_next in range(nS)])
...
def policy_improvement(
    ...
    nS, nA = R_sa.shape
    Q = np.zeros((nS, nA))
    pi_new = np.ndarray(nS, dtype=int)

    # TODO: imüplement Poolicy Evaluation for all states

    for s in range(nS):
        for a in range(nA):
            Q[s, a] = R_sa[s, a] + gamma * sum([T[s, a, s_next] * V[s_next] for s_next in range(nS)])
...
def policy_iteration(
    ...
    while i == 0 or not np.array_equal(pi, pi_improved):
        pi = pi_improved
        V = policy_evaluation(pi, T, R_sa, gamma, epsilon)
        Q, pi_improved = policy_improvement(V, T, R_sa, gamma)
        i += 1
    return Q, pi_improved, i
